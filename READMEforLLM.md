...
- 逼真的软盘驱动器状态灯：
  ...
  - 系统开机时指示灯常亮
  - 读写操作时指示灯闪烁
- 软盘状态保存：使用localStorage保存软盘状态，刷新页面后状态保持不变
...
### 用户插入软盘到B驱动器：
- 用户点击B驱动器插槽或完整软盘图像
- gameController 调用 gameModel.insertFloppyDisk()
- 启动软盘插入动画
- 如果系统开机，驱动器指示灯闪烁
- 动画完成后，调用 model.completeFloppyInsertion()
- 如果系统开机，指示灯变为常亮
- 软盘状态被保存到localStorage

### 用户从B驱动器弹出软盘：
- 用户点击B驱动器的弹出按钮
- gameController 调用 gameModel.ejectFloppyDisk()
- 启动软盘弹出动画
- 如果系统开机，驱动器指示灯闪烁
- 动画完成后，调用 model.completeFloppyEjection()
- 指示灯关闭
- 软盘状态被保存到localStorage

### 页面加载/刷新：
- 立即从localStorage加载软盘状态
- 快速应用软盘视觉状态，避免UI闪烁
- 如果软盘已插入且系统开机，B驱动器指示灯变为常亮
- A驱动器在系统开机状态下始终保持指示灯常亮
...
在 gameModel.js 中扩展 floppyDriveState 对象和相关方法
在 gameView.js 中添加相应的视觉效果和动画
在 gameController.js 中实现用户交互逻辑
