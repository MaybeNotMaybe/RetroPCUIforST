...
class AudioService {
     ...
     * 播放循环音效
     * @param {string} soundId - 音效ID
     * @returns {HTMLAudioElement|null} 音频对象或null
     */
    playLoop(soundId) {
     ...
     * @param {number} [duration=1000] - 淡出时间(毫秒)
     ...
     * 更新音效设置
     * @param {string} soundId - 音效ID
     * @param {string} [newSrc] - 新的音效文件地址
     * @param {number} [newVolume] - 新的音效音量
     */
    ...
    resumeLoopsIfNeeded() {
        ...
        // 检查系统是否开机
        const systemService = window.ServiceLocator.get('system');
        const isSystemOn = systemService ? 
            systemService.isPowerOn() : false;
    ...
    toggleSound(enabled) {
        ...
        // 如果从禁用变为启用，检查是否需要恢复电脑运行声音
            ...
            // 检查系统是否开机
            const systemService = window.ServiceLocator.get('system');
            const isSystemOn = systemService ? systemService.isPowerOn() : false;
    ...
    saveSettings() {
        try {
            ...
            // 使用StorageUtils
            const storageUtils = window.ServiceLocator.get('storage');
            if (storageUtils) {
                storageUtils.save('audioSettings', settings);
            } else {
    ...
    loadSettings() {
        try {
            ...
            const storageUtils = window.ServiceLocator.get('storage');
            if (storageUtils) {
                settings = storageUtils.load('audioSettings');
            } else {
